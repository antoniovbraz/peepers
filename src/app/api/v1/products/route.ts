...
export async function GET(request: NextRequest) {
  try {
    ...
    const mockProducts = [
      ...
      {
        id: "MLB789123456",
        title: "Produto Teste 4 - Relógio Digital",
        price: 199.90,
        status: "paused",
    ...
    const freeShippingFilter = searchParams.get('free_shipping');
    if (freeShippingFilter === 'true') {
      filteredProducts = filteredProducts.filter(p => p.shipping.free_shipping);
    }
    ...
    return NextResponse.json({
      ...
      source: "mock-simplified",
      cache_status: "bypassed",
      timestamp: new Date().toISOString(),
      filters_applied: {
        status: statusFilter || null,
        free_shipping: freeShippingFilter || null
      }
    });
