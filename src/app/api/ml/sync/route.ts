...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      logger.info({ userId: tokenData.user_id }, 'Using access token for user');
      ...
      logger.info({ count: products.length }, 'Sync completed');
...
export async function GET(request: NextRequest) {
  try {
    ...
    logger.error({ err: error }, 'Sync status check failed');
  ...
  try {
    ...
    try {
      ...
      if (lastSync) {
        ...
        if (hoursDiff < 1.5) {
          logger.info({ hoursDiff }, 'Last sync was recently, skipping');
          return NextResponse.json({
      ...
      logger.info({ count: products.length }, 'Cron sync completed');
    ...
    logger.error({ err: error }, 'Cron sync failed');
    await cache.releaseSyncLock();
