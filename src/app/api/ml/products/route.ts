import { NextRequest, NextResponse } from 'next/server';import { NextRequest, NextResponse } from 'next/server';import { NextResponse } from "next/server";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const tokenData = await cache.getUser(`access_token:${knownUserId}`);  try {  try {

    

    if (!tokenData?.access_token) {    console.log('ML Products API called');    const knownUserId = "669073070";

      return NextResponse.json({

        error: "Unauthorized",        const tokenData = await cache.getUser(`access_token:${knownUserId}`);
      ...
      }, { status: 401 });    const tokenData = await cache.getUser(`access_token:${knownUserId}`);    if (!tokenData?.access_token) {
    ...
    if (!tokenData?.access_token) {        success: false,

    const productsResponse = await fetch(`https://api.mercadolibre.com/users/${knownUserId}/items/search?limit=10`, {

      headers: {      return NextResponse.json({        error: "No access token found"

        "Authorization": `Bearer ${tokenData.access_token}`,

        "Accept": "application/json"        error: "Unauthorized",      }, { status: 401 });
